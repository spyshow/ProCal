      ...
      {loading ? (
        <div className="space-y-3" aria-busy="true" aria-label="Loading projects">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="flex items-center gap-4 rounded-xl border border-gray-800/80 bg-gray-900/40 p-4 animate-pulse"
            >
              ...
              <div className="w-20 h-8 bg-slate-800 rounded-lg shrink-0" />
            </div>
          ))}
        </div>
      ) : projects.length === 0 ? (
